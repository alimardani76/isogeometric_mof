...
def main():
    ...
    require([CASES, ELEMENT_CHARGE, PAIR_CHARGE, pair_audit, ads, proc, cif_dir])
    ...
    for i, (_, row) in enumerate(cases.iterrows()):
        ...
        sub = outer[i // 2, i % 2].subgridspec(3, 2, height_ratios=[.58, 3.2, 1.7], hspace=.03, wspace=.05)
        ...
        ax_f.add_patch(FancyBboxPatch((0.0, 0.02), 1.0, 0.96,
                                      boxstyle="round,pad=0.018,rounding_size=0.05",
                                      facecolor=COL["soft"], edgecolor=COL["border"], linewidth=.9))
        metric_chip(ax_f, 0.03, 0.54, 0.28, 0.28, "median |Δlog q|", med_txt, "white", COL["border"])
        metric_chip(ax_f, 0.36, 0.54, 0.27, 0.28, "WC concordance", wc_txt, COL["blue"], COL["blue_dark"])
        metric_chip(ax_f, 0.68, 0.54, 0.24, 0.28, "Selectivity", sel_txt, COL["pink"], COL["pink_dark"])
        ax_f.text(0.03, 0.31, f"A metal q: {summarize_charge(elements, id_a)}", ha="left", va="center",
                  fontsize=7.4, color=COL["muted"], fontweight="bold")
        ax_f.text(0.52, 0.31, f"B metal q: {summarize_charge(elements, id_b)}", ha="left", va="center",
                  fontsize=7.4, color=COL["muted"], fontweight="bold")
        ax_f.text(0.03, 0.10, ROLE_NOTE[role], ha="left", va="center",
                  fontsize=7.05, color=COL["muted"], fontstyle="italic" if role == "functional_motif_example" else "normal")
        if role == "functional_motif_example":
            ax_f.text(0.97, 0.10, "exploratory only", ha="right", va="center",
                      fontsize=7.1, color=ROLE_COLOR[role], fontstyle="italic", fontweight="bold")
        ...
        source_rows.append({
            ...
            "parser_a": backend_a,
            "parser_b": backend_b,
            "charge_use": "case-level element/metal fingerprint only",
        })
    ...
    report = [
        ...
        "- charge is shown only as a case-level metal fingerprint",
        "- no adsorption site, linker-local charge, oxidation state, causal mechanism, or direction is inferred",
    ]
    ...
    manifest = {
        ...
        "inputs": {str(p): sha256(p) for p in [CASES, ELEMENT_CHARGE, PAIR_CHARGE, pair_audit, ads, proc]},
