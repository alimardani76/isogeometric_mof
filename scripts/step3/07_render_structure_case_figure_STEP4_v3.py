...
ROLE_NOTE = {
    "strong_linker_process_aligned": "Large linker contrast with process agreement.",
    "strong_metal_process_aligned": "Large metal-identity contrast with process agreement.",
    "cu_zn_pressure_exception": "Pressure-exception case within the Cu/Zn set.",
    "near_null_comparison": "Matched comparison with minimal adsorption separation.",
    "process_discordant_comparison": "Process translation breaks alignment.",
    "functional_motif_example": "Exploratory only; not class-level evidence.",
}
COL = {
    "blue": "#A7D8EF",
    "blue_dark": "#4C8CB5",
...
def metric_chip(ax, x, y, w, h, label, value, face, edge, value_color=None):
    ...
    ax.text(x + 0.03 * w, y + 0.67 * h, label, ha="left", va="center",
            fontsize=9.9, color=COL["muted"], fontweight="bold")
    ax.text(x + 0.03 * w, y + 0.31 * h, value, ha="left", va="center",
            fontsize=12.3, color=value_color or COL["text"], fontweight="bold")
...
def step4_chemistry_annotation(row, role: str) -> str:
    ...
    if role == "strong_linker_process_aligned":
        return f"robust node | REPEAT W1 all/hetero = {wd_all:.3f}/{wd_het:.3f} e"
    if role == "strong_metal_process_aligned":
        return f"robust coordination | REPEAT W1 all/hetero = {wd_all:.3f}/{wd_het:.3f} e"
    if role == "cu_zn_pressure_exception":
        return f"method-sensitive: {sensitive} sites | REPEAT W1 = {wd_all:.3f}/{wd_het:.3f} e"
    if role == "near_null_comparison":
        return f"near-null | REPEAT W1 all/hetero = {wd_all:.3f}/{wd_het:.3f} e"
    if role == "process_discordant_comparison":
        return f"descriptive context | REPEAT W1 = {wd_all:.3f}/{wd_het:.3f} e"
    if role == "functional_motif_example":
        return f"exploratory | REPEAT W1 all/hetero = {wd_all:.3f}/{wd_het:.3f} e"
    return f"REPEAT W1 all/hetero = {wd_all:.3f}/{wd_het:.3f} e"
...
def main():
    ...
    parser_backends = set()
    source_rows = []
    fig = plt.figure(figsize=(11.2, 14.0))
    outer = fig.add_gridspec(3, 2, hspace=.12, wspace=.08)

    for i, (_, row) in enumerate(cases.iterrows()):
        role = row.selection_category
        accent = ROLE_COLOR[role]
        sub = outer[i // 2, i % 2].subgridspec(3, 2, height_ratios=[.58, 3.05, 2.05], hspace=.015, wspace=.025)
        ...
        ax_h.text(0.0445, 0.50, chr(65 + i), ha="center", va="center", fontsize=21.9, fontweight="bold", color="white")
        ax_h.text(0.10, 0.50, ROLE_TITLE[role], ha="left", va="center", fontsize=16.05, fontweight="bold", color=accent)
        ...
        metric_chip(ax_f, 0.36, 0.62, 0.27, 0.24, "WC concord.", wc_txt, COL["blue"], COL["blue_dark"])
        metric_chip(ax_f, 0.68, 0.62, 0.24, 0.24, "Selectivity", sel_txt, COL["pink"], COL["pink_dark"])
        ax_f.text(0.03, 0.43, f"A mean metal REPEAT q/e: {summarize_charge(elements, id_a)}", ha="left", va="center",
                  fontsize=10.725, color=COL["muted"], fontweight="bold")
        ax_f.text(0.52, 0.43, f"B mean metal REPEAT q/e: {summarize_charge(elements, id_b)}", ha="left", va="center",
                  fontsize=10.725, color=COL["muted"], fontweight="bold")

        s4 = step4.loc[role]
        chem_line = step4_chemistry_annotation(s4, role)
        ax_f.text(0.03, 0.24, chem_line, ha="left", va="center",
                  fontsize=10.725, color=accent, fontweight="bold")

        ax_f.text(0.03, 0.075, ROLE_NOTE[role], ha="left", va="center",
                  fontsize=10.125, color=COL["muted"], fontstyle="italic" if role == "functional_motif_example" else "normal")
        source_rows.append({
            "panel": chr(65 + i),
