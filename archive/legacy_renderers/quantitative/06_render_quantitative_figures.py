...
def panel(ax, letter, title=None, grid=True):
    ax.text(-0.12, 1.11, letter.upper(), transform=ax.transAxes,
...
def figure1():
    ...
    a.axis("off")
    vertical_boxes = [
        (0.25, 0.72, 0.50, 0.14, "Chemistry Changes", C["linker"], C["linker_dark"]),
        (0.25, 0.50, 0.50, 0.14, "Measured geometry constrained", C["control"], C["border"]),
        (0.25, 0.28, 0.50, 0.14, "Adsorption and HOA contrasts", C["metal"], C["metal_dark"]),
    ]
    for x0, y0, w0, h0, txt, face, edge in vertical_boxes:
        ...
        a.text(x0 + w0 / 2, y0 + h0 / 2, txt, ha="center", va="center", fontweight="bold", fontsize=10.4)
    a.annotate("", xy=(0.50, 0.68), xytext=(0.50, 0.64), arrowprops=dict(arrowstyle="-|>", lw=1.2, color=C["muted"]))
    a.annotate("", xy=(0.50, 0.46), xytext=(0.50, 0.42), arrowprops=dict(arrowstyle="-|>", lw=1.2, color=C["muted"]))
    a.annotate("", xy=(0.50, 0.24), xytext=(0.50, 0.20), arrowprops=dict(arrowstyle="-|>", lw=1.2, color=C["muted"]))
    a.add_patch(FancyBboxPatch((0.09, 0.04), 0.82, 0.14,
                               boxstyle="round,pad=0.03,rounding_size=0.03",
                               linewidth=0.9, edgecolor=C["border"],
                               facecolor=C["soft_box"]))
    a.text(0.50, 0.11,
           "Exact topology and dimensionality\nSeven measured geometry controls",
           ha="center", va="center", fontsize=9.6, linespacing=1.20)
...
def figure2():
    ...
    fig.subplots_adjust(left=0.13, right=0.99, bottom=0.11, top=0.94, wspace=0.37, hspace=0.61)
    return fig, [p1, p2, p3]
...
def figure3():
    ...
    fig.subplots_adjust(left=0.14, right=0.99, bottom=0.12, top=0.95, wspace=0.39, hspace=0.66)
    return fig, [p1, p2]
...
def figure4():
    ...
    for i, (head, sub) in enumerate(items):
        ...
        e.text(x0 + 0.085, 0.39, sub, ha="center", va="center", fontsize=8, fontweight="bold", color=C["muted"], alpha=0.7)
