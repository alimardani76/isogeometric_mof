"""Shared OVITO helpers for Panel D Round 1."""

from pathlib import Path
import numpy as np

from d_config import (
    D_PAIRWISE_CUTOFFS, PALETTE, RADII, BOND_WIDTH,
...
def build_pipeline(cif_path, hide_h=True):
    ...
    # OVITO-version-compatible particle styling.
    def setup_types(frame,data):
        prop=data.particles_.particle_types_
        for t in prop.types_:
            name=t.name
            if name in RADII:
                t.radius=float(RADII[name])
            if name in PALETTE:
                t.color=tuple(PALETTE[name])
    ...
    if not hasattr(CreateBondsModifier.Mode,"Pairwise") or not hasattr(bonds,"set_pairwise_cutoff"):
        raise RuntimeError(
            ...
            "Use the same OVITO environment that successfully ran the Panel-B v3+ scripts."
        )
    ...
    for (a,b),cut in D_PAIRWISE_CUTOFFS.items():
