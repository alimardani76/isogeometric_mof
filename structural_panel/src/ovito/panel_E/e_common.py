"""Shared OVITO helpers for Panel E."""

from pathlib import Path
import numpy as np

from e_config import (
    E_PAIRWISE_CUTOFFS, PALETTE, RADII,
...
def build_pipeline(cif_path,hide_h=True):
    ...
    if not hasattr(CreateBondsModifier.Mode,"Pairwise") or not hasattr(bonds,"set_pairwise_cutoff"):
        raise RuntimeError(
            ...
            "Use the same ovito_render environment that ran Panels A/B/D."
        )
...
def render_pair(pi,di,pii,dii,oi,oii):
    fov=max(auto_fov(pi,di),auto_fov(pii,dii))*PAIR_MARGIN
    render(pi,di,fov,oi)
