...
from .raspa_publication import (
    publication_statistics, PAIR_ORDER, RAW_STATS_FILE, PAIR_SUMMARY_FILE,
    ORIENTATION_FILE, STRUCTURAL_TABLE_FILE,
)
...
def render_s06():
    """Detailed eight-pair RASPA SI figure in publication-canonical orientation."""
    apply_style(9.25)
    ...
    gC = energy_panel(c, "gcmc_0p1bar", "C", "Interaction-energy terms · 0.1 bar")
    gD = energy_panel(d, "gcmc_1bar", "D", "Interaction-energy terms · 1 bar")
    ymin, ymax = d.get_ylim(); d.set_ylim(ymin, ymax + 0.18 * (ymax - ymin))
    ...
    write_manifest(fig_dir, "Figure_S06", inputs, outputs, [sA, sB, sC, sD], {
        "orientation": "Figure 5/Table S09 convention enforced for six structural cases",
        "energy_boundary": "classical force-field interaction-energy terms; not a thermodynamic free-energy decomposition",
        "exclusions": "no RASPA density map; raw grids/generation code unavailable",
    })
    return outputs
