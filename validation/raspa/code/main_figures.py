...
from .raspa_publication import (
    publication_statistics, correlation_summary, PAIR_ORDER,
    RAW_STATS_FILE, PAIR_SUMMARY_FILE, ORIENTATION_FILE, STRUCTURAL_TABLE_FILE,
)
...
def render_figure_06():
    """Main-text RASPA challenge figure using publication-canonical A/B labels.

    Panel A intentionally uses orientation-independent contrast magnitude. Panels
    B-C use the Figure 5/Table S09 A/B convention, allowing direction-sensitive
    interpretation without arbitrary label reversals.
    """
    apply_style(9.7)
    stats = publication_statistics(write=True)
    ...
    corr = correlation_summary(stats)

    fig = plt.figure(figsize=(12.1, 8.3))
    gs = fig.add_gridspec(2, 2, width_ratios=[1.18, 1.0], wspace=0.34, hspace=0.36)
    a = fig.add_subplot(gs[:, 0])
    b = fig.add_subplot(gs[0, 1])
    c = fig.add_subplot(gs[1, 1])

    # A — orientation-independent magnitude: strongest clean summary for main text.
    title_left(a, "Selected contrast magnitude across loading regimes"); panel_label(a, "A", x=-0.23, y=1.045)
    ...
    for mode in modes:
        ...
        x = g.absolute_log2_contrast.to_numpy(float)
        lo = g.absolute_log2_ci_low.to_numpy(float)
        hi = g.absolute_log2_ci_high.to_numpy(float)
        col = mode_color[mode]
        a.errorbar(
            x, yy + offsets[mode], xerr=[x - lo, hi - x], fmt=markers[mode],
            ms=5.4, color=col, ecolor=col, capsize=2.2, lw=1.08,
            label=mode_lab[mode], zorder=3,
        )
    a.axvline(1.0, color=COLORS["grid"], ls="--", lw=0.95, zorder=1)
    a.text(1.0, -0.53, "2×", ha="center", va="bottom", fontsize=8.4, color=COLORS["muted"])
    a.set_yticks(yy, [ROLE_LABEL.get(roles.loc[p, "role"], p) for p in pair_order]); a.invert_yaxis()
    a.set_xlabel(r"Magnitude of adsorption contrast · $|\log_2(A/B)|$")
    a.set_xlim(left=0)
    clean_axis(a, "x")
    a.legend(frameon=False, loc="lower right", borderaxespad=0.35, ncol=1, handletextpad=0.5, labelspacing=0.45)

    # B — direction-sensitive persistence after canonicalization.
    ...
    for mode, mark in [("gcmc_0p1bar", "s"), ("gcmc_1bar", "^")]:
        ...
        x = h.loc[pair_order, "signed_log2_A_over_B"].to_numpy(float)
        yv = g.loc[pair_order, "signed_log2_A_over_B"].to_numpy(float)
        label = f"{mode_lab[mode]} · r={corr[mode]['pearson_r']:.3f}"
        b.scatter(x, yv, s=45, marker=mark, facecolor="white", edgecolor=mode_color[mode], lw=1.35, label=label, zorder=3)
        if mode == "gcmc_1bar":
            offsets_text = {"A1": (5, 4), "A3": (5, 4), "A4": (5, -12)}
    ...
    clean_axis(b, None)
    b.legend(frameon=False, loc="upper left", borderaxespad=0.35, handletextpad=0.5, labelspacing=0.4)

    # C — sensitivity experiment, not a physical charge-state material.
    ...
    for yi, p in enumerate(pair_order):
        ...
        line_w = 3.1 if p == "A4" else 2.6
        c.plot([hc.loc[p, "signed_log2_A_over_B"], hf.loc[p, "signed_log2_A_over_B"]], [yi, yi],
               color=line_col, lw=line_w, solid_capstyle="round")
    c.scatter(hc.signed_log2_A_over_B, np.arange(len(pair_order)), s=46, facecolor="white",
              edgecolor=COLORS["raspa_chargeoff"], lw=1.35, label="Charge off", zorder=3)
    ...
    a4y = pair_order.index("A4")
    c.annotate("charge-sensitive", xy=(hf.loc["A4", "signed_log2_A_over_B"], a4y), xytext=(8, -12),
               textcoords="offset points", fontsize=8.2, color=COLORS["metal"])

    fig.subplots_adjust(left=0.245, right=0.985, top=0.935, bottom=0.105)
    ...
    wide = []
    for p in pair_order:
        row = {"pair_id": p, "henry_signed_log2": h.loc[p, "signed_log2_A_over_B"]}
        for mode in ["gcmc_0p1bar", "gcmc_1bar"]:
            gg = stats[stats["mode"] == mode].set_index("pair_id")
            row[f"{mode}_signed_log2"] = gg.loc[p, "signed_log2_A_over_B"]
            row[f"{mode}_pearson_r_all_8_pairs"] = corr[mode]["pearson_r"]
        wide.append(row)
    sB = write_panel_source(pd.DataFrame(wide), fig_dir, "B")
    sC = write_panel_source(stats[(stats["mode"].isin(["henry_full", "henry_chargeoff"])) & stats.pair_id.isin(pair_order)], fig_dir, "C")
    inputs = [RAW_STATS_FILE, PAIR_SUMMARY_FILE, ORIENTATION_FILE, STRUCTURAL_TABLE_FILE]
    write_manifest(fig_dir, "Figure_06", inputs, outputs, [sA, sB, sC], {
        "raspa": "RASPA 3.0.29 frozen closure results; six structural cases + two additional linker comparators; no density map",
        "orientation": "Figure 5/Table S09 convention enforced for six structural cases before all signed plotting/statistics",
        "panel_A": "orientation-independent contrast magnitude",
        "panel_B_C": "publication-canonical signed A/B convention",
        "energy_decomposition": "moved to Figure S06 to avoid over-interpreting force-field energy components",
    })
    return outputs
